...
class Generate(object):
    ...
    #@staticmethod
    def __choose_pivotal(self, pivotal_net, h_ctx, ctx_idx, focus_threshold, choose_max, surf_mask=None):
        ...
        if choose_max:
            ...
            if isinstance(surf_mask, torch.Tensor) and surf_mask.sum() > 0:
                ...
                surf_pivotal_mask = pivotal_prob_surf > focus_threshold
                ...
                pivotal_mask = (pivotal_prob >= focus_threshold).view(-1)
    ...
    def choose_pivotal(self, h_cpx, cpx_index, idx_ligand_ctx_in_cpx, data, atom_idx):
        if atom_idx == 0:
            ...
            pivotal_idx_, pivotal_prob = self.__choose_pivotal(
                ...
                self.focus_threshold, self.choose_max, 
                surf_mask=surf_mask)
        else:
            pivotal_idx_, pivotal_prob = self.__choose_pivotal(
...
# Parameters
            ...
            number_of_output = 0
            mol_dict = {}
            #f_Modify = Filter(sa_threshhold=5.0, qed_threshhold=0.6, mw_threshhold=(350,500))     # Mpro first time
            f_Modify = Filter(sa_threshhold=5.0, qed_threshhold=0.4, mw_threshhold=(350,500))       # Mpro_second_time_2
            while number_of_output < num_gen:   
                i = i + 1 
                ...
                filter_1 = f_Modify._filter(Chem.MolFromSmiles(s))  # True or False
                if filter_1:
