...
from .chemical_bond_generator import BondFlow
...
edge_flow_cfg = EasyDict(
    {'edge_channels':8, 'num_filters':[32,8], 'num_bond_types':3,
     'num_heads':2, 'cutoff':10.0, 'num_st_layers':3}
    )
config = EasyDict(
...
class pocket_strmod_WithEdge(nn.Module):
    def __init__(self, config) -> None:
        super(pocket_strmod_WithEdge, self).__init__()
        self.config = config
        ...
        self.edge_flow = BondFlow(
            ...
            num_st_layers=config.edge_flow.num_st_layers, bottleneck=config.bottleneck,
            use_conv1d=config.use_conv1d
            )
    ...
    def get_loss(self, data, annealing=False):
        ...
        pos_fake_pred, _ = self.edge_flow.pos_classfier(
        ...
        pos_real_pred, _ = self.edge_flow.pos_classfier(
