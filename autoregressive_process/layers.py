...
class GlobalAttention(Module):
    def __init__(self, in_sca, in_vec, out_sca, out_vec, bottleneck=1, use_conv1d=False) -> None:
        ...
        self.lin_gate = nn.Sequential(
            [GBPerceptronVN(in_sca, in_vec, out_sca, out_vec, bottleneck=bottleneck, use_conv1d=use_conv1d),
            GBLinear(in_sca, in_vec, out_sca, out_vec, bottleneck=bottleneck, use_conv1d=use_conv1d)])
        self.lin_theta = nn.Sequential(
            [GBPerceptronVN(in_sca, in_vec, out_sca, out_vec, bottleneck=bottleneck, use_conv1d=use_conv1d),
            GBLinear(in_sca, in_vec, out_sca, out_vec, bottleneck=bottleneck, use_conv1d=use_conv1d)])
        '''self.gbp_out = GBPerceptronVN(in_sca, in_vec, out_sca, out_vec, bottleneck=bottleneck, use_conv1d=use_conv1d)'''

    def forward(self, h_cpx, idx_ligand_ctx_in_cpx, ligand_ctx_step):

        h_lig_ctx = [
            h_cpx[0][idx_ligand_ctx_in_cpx],
            h_cpx[1][idx_ligand_ctx_in_cpx]
            ]

        N_lig_ctx = h_lig_ctx[0].size(0)

        x_k = self.lin_gate(h_lig_ctx)
        '''x_k = [
            k[0].view(N_lig_ctx, self.num_heads, -1),
            k[1].view(N_lig_ctx, self.num_heads, -1, 3)
            ]'''
        #alpha_scale = [x_i[0].size(-1)**0.5, x_i[1].size(-2)**0.5]
        gate = [
            x_k[0].sum(-1), #/alpha_scale[0] # (N', heads)
            x_k[1].sum(-1).sum(-1) #/alpha_scale[1] # (N', heads)
            ]
        gate = [
            scatter_softmax(gate[0], ligand_ctx_step, dim=0),
            scatter_softmax(gate[1], ligand_ctx_step, dim=0)
            ]
        
        theta = self.lin_theta(h_lig_ctx)
        '''theta = [
            theta[0].view(N_lig_ctx, self.num_heads, -1),
            theta[1].view(N_lig_ctx, self.num_heads, -1, 3)
            ]'''
        out = [
            (gate[0].unsqueeze(-1) * theta[0]).view(N_lig_ctx, -1),
            (gate[1].unsqueeze(-1).unsqueeze(-1) * theta[1]).view(N_lig_ctx, -1, 3)
            ]
        out = [
            scatter_sum(out[0], ligand_ctx_step, dim=0), #, dim_size=N_lig_ctx),
            scatter_sum(out[1], ligand_ctx_step, dim=0)  #, dim_size=N_lig_ctx)
            ]
        return out
