...
from .protein_residues import RESIDUES_TOPO
try:
    import pymol
except:
...
class Atom(object):
    def __init__(self, atom_info):
        ...
        self.charge = atom_info[78:80].strip()
        if self.element == 'SE':
            self.element = 'S'
            self.name = 'SD'
            self.res_name = 'MET'
    ...
    def to_string(self):
        ...
        out = fmt.format('ATOM', self.idx, self.name, '', self.res_name,
                       ...
                       self.temperature_factor, self.element, self.charge)
        return out
