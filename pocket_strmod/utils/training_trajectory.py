...
from .knn_search import count_neighbors, mask_node, get_bfs_perm, get_rfs_perm, make_pos_label,\
...
class pivotalMaker(object):
    ...
    def run(self, data):
        if data.ligand_context_pos.size(0) == 0:
            ...
            edge_index_query = torch.stack([data.edge_query_index_0, data.edge_query_index_1])
            data.index_real_cps_edge_for_atten, data.tri_edge_index, data.tri_edge_feat = get_tri_edges(edge_index_query, data.y_pos, data.context_idx, data.ligand_context_bond_index, data.ligand_context_bond_type)
        return data
...
class Combine(object):
    def __init__(self, lig_traj, pivotal_maker, atom_composer, lig_only=False):
...
COLLATE_KEYS = [
                ...
                'pos_query_knn_edge_idx_1', 'pivotal_label', 'ligand_pivotal', 'y_pos', 'edge_label', 'atom_label', 'edge_query_index_0', 
...
def collate_fn(data_list, collate_keys=COLLATE_KEYS):
    ...
    for idx, d in enumerate(data_list):
        ...
        data_dict['ligand_pivotal'].append(d.ligand_pivotal)
        ...
        data_dict['ligand_ctx_step'].append(torch.zeros_like(d.context_idx) + idx - 1)
    ...
    data_dict['ligand_pivotal'] = torch.cat(data_dict['ligand_pivotal'])
    ...
    data_dict['ligand_ctx_step'] = torch.cat(data_dict['ligand_ctx_step'])
