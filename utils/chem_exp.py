...
def add_ligand_atom_to_data(
    ...
    max_valence_dict=max_valence_dict, remove_tri_ring=True):
    data = old_data.clone()
    ...
    data.ligand_context_feature_full = torch.cat([
        ...
        torch.cat([
            F.one_hot(element.view(1), len(type_map)).to(data.ligand_context_feature_full), 
            torch.tensor([[1, 0, 0]]).to(data.ligand_context_feature_full),  
            torch.tensor([[0, 0, 0]]).to(data.ligand_context_feature_full)  
        ], dim=1)
    ], dim=0)
    ...
    if len(bond_type) != 0:
        ...
        data.ligand_context_feature_full[bond_index[1, :], idx_num_neigh] += 1 
        data.ligand_context_feature_full[bond_index[1, :], idx_valence] += bond_type 
        data.ligand_context_feature_full[bond_index[1, :], idx_num_of_bonds + bond_type - 1] += 1   
        # the new atom
        data.ligand_context_feature_full[-1, idx_num_neigh] += len(bond_index[1]) 
        data.ligand_context_feature_full[-1, idx_valence] += torch.sum(bond_type) 
        for bond in [1, 2, 3]:
            data.ligand_context_feature_full[-1, idx_num_of_bonds + bond - 1] += (bond_type == bond).sum() 
...
def modify_submol(mol):  
    ...
    for fragment in sub_fragments:
        ...
        mol.GetAtomWithIdx(idx_atom_N).SetFormalCharge(1)  
        mol.GetAtomWithIdx(idx_atom_O).SetFormalCharge(-1) 
    return mol
...
##################################### parameter ######################################################################################
    nomodify_l = {0,1,2,3,4,5}    # the initial time
    #nomodify_l = {1,2,5,6,14,15}   # the second time
##################################### parameter ######################################################################################

    p1 = Chem.MolFromSmarts('[#6,#7]=[#6]1-[#6,#7]~&@[#6,#7]~&@[#6,#7]~&@[#6,#7]~&@[#6,#7]-1')
    p1_ = Chem.MolFromSmarts('[C,N]=[C]1-[C,N]~&@[C,N]~&@[C,N]~&@[C,N]~&@[C,N]-1')
    Chem.MolFromSmarts('[#6,#7]=[#6]1~&@[#6,#7,#8,#16]~&@[#6,#7,#8,#16]~&@[#6,#7,#8,#16]~&@[#6,#7,#8,#16]~&@1')
    Chem.MolFromSmarts('[C,N,S]=[C]1~&@[C,N,O,S]~&@[C,N,O,S]~&@[C,N,O,S]~&@[C,N,O,S]~&@1')
    ...
    for p2 in PATTERNS_1:
        ...
        for sub in subs2:
            ...
            ###########################################################################################################################
            if nomodify_l.issubset(set(sub)):
                continue
            ###########################################################################################################################
...
def check_alert_structure_mols(mol, pattern_mol_list):
    Chem.GetSSSR(mol)
    mol.UpdatePropertyCache()
    for p in pattern_mol_list:
        subs = mol.GetSubstructMatches(p)
        if len(subs) != 0:
            return True
    else:
